...
- **Multi-Window Layout**: The Command Center features a CSS grid with 7 category windows (URGENT, DIRECTIVES, SCENES, PARTNER WATCH, CONNECTION, STRUCTURE, JOURNAL), each with varied sizes, specific color-coded borders/glow, and scrollable content areas. Punishments and Rewards are displayed as standalone "Consequence Panels" between Protocol Scroll and Command Center — separate from the CC grid. Dares appear in both consequence panels and SCENES window. Achievements/stickers appear in CONNECTION window.
- **Consequence Panels**: Two side-by-side panels above the Command Center showing punishments (blood red, with dares) and rewards (dark gold). Component: `client/src/components/consequence-panels.tsx`. Sub role gets action buttons (Done/Redeem); Dom role gets Arsenal/Chest links.
